            ...
            return (
              ...
              >
                <div className="relative z-10">
                  ...
                  {item.badge && (
                    <span className="absolute -top-1 -right-1 bg-gradient-to-r from-pink-500 to-purple-500 text-white text-xs font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1 shadow-lg">
                      {item.badge}
                    </span>
                  )}
                </div>
                <span className={`text-xs font-medium z-10 ${active ? 'font-semibold' : ''}`}>
                  {item.label}
                </span>
              </button>
            );
